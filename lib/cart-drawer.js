...
(function () {
  ...
  // GET /cart to obtain one. This eliminates the race between a load-time
  // fetch and a fast customer click.
  var _tokenPromise = null;

  function ensureToken() {
    if (wcApi.getStoredCartToken()) return Promise.resolve();
    if (_tokenPromise) return _tokenPromise;
    _tokenPromise = wcApi.wcGet('cart')
      .then(function () { _tokenPromise = null; })
      .catch(function () { _tokenPromise = null; });
